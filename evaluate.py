...
_parser.add_argument("--run_name",   type=str, default=None,          help="Short label (e.g. baseline, V2, V3)")
_parser.add_argument("--save_dir",   type=str, default="result/eval")
...
# Loads harmonizer.pt from the same checkpoint directory as the LoRA weights
# and replaces pipe.transformer.forward so the denoising loop follows the
# V2 training tensor path:
...
# ── Composite image helper ───────────────────────────────────────────────────
def make_composite(ref_img, target_img, pred_img, h=512):
    """Stack 3 images side-by-side at the same height with separator lines."""
    def _resize_h(img, height):
        ratio = height / img.shape[0]
        return cv2.resize(img, (max(1, int(img.shape[1] * ratio)), height))

    panels = [_resize_h(img, h) for img in (ref_img, target_img, pred_img)]
...
for cls in TEST_CLASSES:
    ...
    for fname in files:
        ...
        try:
            ...
            composite = make_composite(ref_image, gt, pred)
            cv2.imwrite(os.path.join(composite_cls, fname), cv2.cvtColor(composite, cv2.COLOR_RGB2BGR))
            ...
            all_rows.append({
                ...
                "filename": fname,
                "SSIM_bg":  round(s,      4),
                "LPIPS":    round(lp_full, 4),
                "LPIPS_fg": round(lp_fg,   4),
                "DINO":     round(dino_s,  4),
                "CLIP_I":   round(clip_s,  4),
                "pred_path": os.path.join(save_cls, fname),
            })
        except Exception as e:
            print(f"  [ERROR] {fname}: {e}")
            all_rows.append({
                "run": _run_name, "class": cls, "filename": fname,
                "SSIM_bg": None, "LPIPS": None, "LPIPS_fg": None,
                "DINO": None, "CLIP_I": None, "pred_path": "ERROR",
            })
...
for cls in TEST_CLASSES:
    ...
    summary_rows.append({
        ...
        "LPIPS":    round(sub["LPIPS"].mean(),     4),
        "LPIPS_fg": round(sub["LPIPS_fg"].mean(),  4),
        "DINO":     round(sub["DINO"].mean(),      4),
        "CLIP_I":   round(sub["CLIP_I"].mean(),    4),
    })
ok = df.dropna(subset=["SSIM_bg"])
summary_rows.append({
    "run": _run_name, "class": "ALL",
    "n_ok":     len(ok),
    "SSIM_bg":  round(ok["SSIM_bg"].mean(),  4),
    "LPIPS":    round(ok["LPIPS"].mean(),     4),
    "LPIPS_fg": round(ok["LPIPS_fg"].mean(),  4),
    "DINO":     round(ok["DINO"].mean(),      4),
    "CLIP_I":   round(ok["CLIP_I"].mean(),    4),
})
df_summary = pd.DataFrame(summary_rows)

excel_path = os.path.join(SAVE_DIR, f"metrics_{_run_name}.xlsx")
